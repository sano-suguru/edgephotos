...
この文書は、v1 を完成させるための実装順序と各段階の到達点だけを管理します。設計の理由は [decisions.md](decisions.md)、不変条件は [architecture.md](architecture.md) と [security.md](security.md) を参照してください。
...
- D1 migration と private R2 binding が利用できる
- OpenAPI を生成できる
- local / remote-test / production が分離されている
- shadcn/ui + Base UI の主要 component が Preact production build で成立する
...
- original / thumbnail / preview が想定した経路で保存・取得できる
- refresh 後も状態が一貫する
