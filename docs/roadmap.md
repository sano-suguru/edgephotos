...
- ⬜ original の checksum 付き PUT（[D-018](decisions.md)）を実 R2 で確認する。正しい bytes は `200`、digest 違いは `400` で object なし、binding の `head().checksums.sha256` が読める。remote-test の CORS に `x-amz-checksum-sha256` を追加したうえで Browser からも 1 枚 upload する
