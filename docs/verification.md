...
- 続く review を受けて、「状態」に出すアップロードを期限切れのものだけにし（「中断したアップロード」）、削除処理中の行を外した（件数は「中断した完全削除」の欄に出る）
- `e2e/manage.spec.ts` の、「メンテナンス」を開いて終わる test が、次の test の失敗として `route.fetch: Test ended` を出すことがあった。diagnostics の差し替えが処理中のまま test が終わっていた。応答を待ってから終えるようにし、3 回続けて通ることを見た

`e2e/manage.spec.ts`（`e2e/library.spec.ts` から名前を変えた）は次を確かめる。文言の全文ではなく、行・リンク・ボタンの有無と移動先を見る。

- 下部ナビに「ライブラリ」のリンクが無く、「管理」がある
- 対応の要るものが無い「管理」は、写真とアルバムの行、件数付きのゴミ箱へのリンクだけ。進行中のアップロードがあっても、アップロード・削除処理中・ゴミ箱の行と「メンテナンスを開く」は出ない
- 期限切れのアップロードがあると「中断したアップロード」の行が出て、「メンテナンスを開く」から「メンテナンス」へ移れる。止まった完全削除は「中断した完全削除」の欄にだけ出る
