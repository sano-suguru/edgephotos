...
- WebKit で full page の screenshot を撮ると、fixture が `style-src-elem` の違反を報告した。同じ 2 画面を通る `e2e/mobile.spec.ts` は、CSP の検査を付けたまま違反 0 件で通った。screenshot の撮影時に Playwright が差し込む style と見ているが、確かめてはいない

`e2e/manage.spec.ts`（`e2e/library.spec.ts` から名前を変えた）は、下部ナビに「ライブラリ」のリンクが無く「管理」があること、「管理」に件数とゴミ箱があり、バックアップ処理の日時・点検・作り直し・書き出しが無いこと、「管理」から「メンテナンス」へ移れて、そこに前の画面と同じ説明と操作があることを確かめる。書き出しは実際にダウンロードし、点検は「要対応」の文字を確かめる。両画面の文字に運用の用語と「管理者」「権限」が無いことも見る。もう 1 つの test は、`/settings/maintenance` を直接開き、点検と書き出しが使えることを確かめる。`e2e/mobile.spec.ts` は phone で「管理」→「メンテナンス」→「管理」と移る。
