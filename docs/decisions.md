...
**記録の意味が正しくなる。** export のページは backup 以外からも最後まで読まれます。

- `pnpm backup restore` は、書き込み先のライブラリが空かを確かめるため
- `pnpm backup verify` は、backup と比べるため
- ライブラリ画面は、manifest をダウンロードするため
...
- 最初の authenticator の登録には MFA が要らない（[Independent MFA](https://developers.cloudflare.com/cloudflare-one/access-controls/access-settings/independent-mfa/)）。member が登録する前と、紛失で authenticator を削除した後は、email を乗っ取った攻撃者が先に登録できる（対処は [MFA を足す](operations.md#mfa-を足す推奨)）
...
## D-039: 「ライブラリ」タブを「管理」にし、保守の機能を一段奥の「メンテナンス」へ分ける

**状態:** 採用（2026-09-26）

「ライブラリ」画面には、家族が日常的に見る件数やゴミ箱と、ほとんど使わない保守の機能（バックアップ処理の完了日時、保存状態の点検、サムネイルの作り直し、中断したアップロードの整理、写真とアルバムの情報の書き出し）が同じ画面に並んでいました。#58 で運用の用語を除いても、普段の確認に要らない説明とボタンが画面の大半を占めていました。

**下部ナビ（desktop ではヘッダー）のタブを「管理」にし、保守の機能を「管理」から開く「メンテナンス」（`/settings/maintenance`）へ移す。** 「メンテナンス」はタブにしない。

- タブ名を「管理」にした理由: この画面は写真の一覧ではなく、件数・ゴミ箱・アップロードの状態を見て扱う場所。「ライブラリ」は EdgePhotos では写真全体（library）を指す語でもあり、タブ名に使うと両者が紛れる。本文で写真全体を指す「ライブラリ」（整理の説明など）は変えない。タブのアイコンも、旧ライブラリの円柱から調整を表すスライダーに替えた（「メンテナンス」はレンチ）
- 「管理」に残すもの: 写真・アルバムの件数、ゴミ箱へのリンク（件数を添える。同じ件数の行は重ねない）、中断したアップロードの件数、中断した完全削除の「削除を再開」。「削除を再開」は、member がゴミ箱から始めた完全削除を最後まで終わらせる操作で、日常の操作の続きにあたるため残す
- 正常なら黙る: 「管理」に出すのは、対応が要る状態だけにする。毎回「0」を読ませる行は、家族にとって情報にならない。行が現れること自体を知らせにする
  - アップロードは、期限（600 秒）を過ぎても終わらなかったもの（`expiredUploads`）だけを「中断したアップロード」として出す。進行中のもの（`pending` のうち期限内）は正常な途中の状態なので出さない。`pending` という保存上の分類を、そのまま利用者の語にしない
  - 止まった完全削除は「中断した完全削除」の欄が件数とともに出すので、「状態」に行を重ねない
- 「メンテナンス」へ移すもの: バックアップ処理の完了日時と説明、保存状態の点検（点検結果、「サムネイルを作り直す」、「中断したアップロードを整理する」）。どれも異常時や運用の確認でだけ使う。中断したアップロードがあるときは、「管理」の説明に「メンテナンスを開く」のリンクを添える。次にすることが決まっているので、画面名を覚えさせずにそこへ連れて行く
- 名前を「メンテナンス」にした理由: 「普段は触らない保守の機能」という分類を表す。「管理者向け」「詳細設定」のような名前は、使える人が限られる、または設定を変える場所だと読めてしまう。見出し「ストレージの点検」も、利用者向けの語として「保存状態の点検」にした

両画面に権限の差はない。分離はアクセス制御ではなく、情報設計の分類です。household の member は全員が対等で（[D-028](#d-028-許可した複数の-email-が-1-つの-library-を対等に共同利用する)）、どちらの画面もどのボタンも同じように使える。role や member ごとの権限は足さず、API・backup・manifest・storage audit・repair の挙動と形式も変えていない。「メンテナンス」の URL を直接開いても同じように使える。

Web UI の「写真とアルバムの情報をダウンロード」は「メンテナンス」へ移さず、廃止した（[D-040](#d-040-web-ui-の-manifest-ダウンロードを廃止する)）。

再検討する条件: 「メンテナンス」の機能を家族が日常的に使う必要が出た場合（たとえば点検で見つかる問題が頻繁に起きる場合）。そのときは原因を先に直し、それでも要る機能だけを「管理」へ戻す。

「中断したアップロード」は、期限（600 秒）を過ぎた upload（`expiredUploads`）を中断とみなす、今の仕様に依存した名前です。大きなファイルを長時間かけて送るなど、期限を過ぎても正常に続く upload を許すように変えた場合は、この前提と名前を見直す。

## D-040: Web UI の manifest ダウンロードを廃止する

**状態:** 採用（2026-09-26）

**Web UI の「写真とアルバムの情報をダウンロード」は廃止する。** D-039 で「メンテナンス」へ移すつもりだったが、何のために押すのかを説明できなかった。画面の分離（D-039）とは別に覆りうる判断なので、分けて記録する。

- この操作で得られる JSON には写真本体が含まれず、単体では backup にも restore にもならない
- Web から取得した manifest を使う具体的な操作・移行手順・復元手順は、定義されていない。初期の要件（roadmap の Feature 5）は restore と検証のための export の中身を定めたもので、Web のボタンを求めていなかった
- 同じ形式の manifest は、`pnpm backup export` が写真本体とともに保存する。Export API（`GET /api/v1/export/*`）は backup / verify / restore のために残す
