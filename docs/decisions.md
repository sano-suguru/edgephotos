...
**1 枚 = 1 request = 1 つの結果とする。** batch は部分成功として扱います。10 枚のうち 1 枚が失敗しても、成功した 9 枚は取り消しません。失敗した asset だけを選び直して再試行できます。all-or-nothing にはしません。D1 と R2 をまたいで巻き戻す仕組みを作らないという既存の方針（[D-012](#d-012-finalize-の保存確認は存在サイズ形式派生画像-metadata-とする)、[D-023](#d-023-d1-と-r2-の突合は-owner-が実行し自動で消すのは中断した-upload-の残りだけにする)）と同じです。
