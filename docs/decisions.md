...
- タブ名を「管理」にした理由: この画面は写真の一覧ではなく、件数・ゴミ箱・アップロードの状態を見て扱う場所。「ライブラリ」は EdgePhotos では写真全体（library）を指す語でもあり、タブ名に使うと両者が紛れる。本文で写真全体を指す「ライブラリ」（整理の説明など）は変えない
- 「管理」に残すもの: 写真・アルバムの件数、ゴミ箱へのリンク（件数を添える。同じ件数の行は重ねない）、未完了のアップロード（うち期限切れ）、削除処理中の件数、中断した完全削除の「削除を再開」。「削除を再開」は、member がゴミ箱から始めた完全削除を最後まで終わらせる操作で、日常の操作の続きにあたるため残す。止まったままだと件数が合わない状態が「管理」に出続ける
- 「メンテナンス」へ移すもの: バックアップ処理の完了日時と説明、保存状態の点検（点検結果、「サムネイルを作り直す」、「中断したアップロードを整理する」）、写真とアルバムの情報の書き出し。どれも異常時や運用の確認でだけ使う。期限切れのアップロードがあるときは、「管理」の説明に「メンテナンスを開く」のリンクを添える。次にすることが決まっているので、画面名を覚えさせずにそこへ連れて行く
...
- 「写真とアルバムの情報をダウンロード」は削除せず、「メンテナンス」へ移した。CLI の backup でも同じ情報を得られるが、CLI を使わない member が Web から取り出せる手段として残す
