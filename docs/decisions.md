...
- 「管理」に残すもの: 写真・アルバム・ゴミ箱の件数、ゴミ箱へのリンク、未完了のアップロード（うち期限切れ）、削除処理中の件数、中断した完全削除の「削除を再開」。「削除を再開」は、member がゴミ箱から始めた完全削除を最後まで終わらせる操作で、日常の操作の続きにあたるため残す。止まったままだと件数が合わない状態が「管理」に出続ける
- 「メンテナンス」へ移すもの: バックアップ処理の完了日時と説明、保存状態の点検（点検結果、「サムネイルを作り直す」、「中断したアップロードを整理する」）、写真とアルバムの情報の書き出し。どれも異常時や運用の確認でだけ使う。期限切れのアップロードがあるときは、「管理」の説明から「メンテナンス」の点検を案内する
