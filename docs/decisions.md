...
## D-039: 「ライブラリ」タブを「管理」にし、保守の機能を「メンテナンス」へ分け、情報の書き出しを Web から外す
...
- 「管理」に残すもの: 写真・アルバムの件数、ゴミ箱へのリンク（件数を添える。同じ件数の行は重ねない）、未完了のアップロード（うち期限切れ）、削除処理中の件数、中断した完全削除の「削除を再開」。「削除を再開」は、member がゴミ箱から始めた完全削除を最後まで終わらせる操作で、日常の操作の続きにあたるため残す
- 正常なら黙る: 未完了のアップロードと削除処理中の行は、1 件以上あるときだけ出す。毎回「0」を読ませる行は、家族にとって情報にならない。行が現れること自体を知らせにする
- 「メンテナンス」へ移すもの: バックアップ処理の完了日時と説明、保存状態の点検（点検結果、「サムネイルを作り直す」、「中断したアップロードを整理する」）。どれも異常時や運用の確認でだけ使う。期限切れのアップロードがあるときは、「管理」の説明に「メンテナンスを開く」のリンクを添える。次にすることが決まっているので、画面名を覚えさせずにそこへ連れて行く
...
**Web UI の「写真とアルバムの情報をダウンロード」は廃止する。** 最初は「メンテナンス」へ移すつもりだったが、何のために押すのかを説明できなかった。
