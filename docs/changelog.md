...
ライブラリ画面から CLI・Cloudflare の用語を除きました。「最終 backup export」は「バックアップの完了日時」、「Export」と「manifest をダウンロード」は「写真とアルバムの情報を書き出す」と「写真とアルバムの情報をダウンロード」になりました。「Migration」の行は画面から外しました（`pnpm diagnose` の `worker: D1 schema` で確認します）。API・backup・manifest の形式は変わりません。
