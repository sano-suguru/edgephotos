...
下部ナビの「ライブラリ」タブを「管理」にし、保守の機能を「管理」から開く「メンテナンス」画面（`/settings/maintenance`）へ移しました（[D-039](decisions.md)）。「管理」には件数・ゴミ箱・未完了のアップロード・削除処理中と「削除を再開」が残ります。「メンテナンス」には、バックアップ処理の完了日時、保存状態の点検（旧「ストレージの点検」、作り直しと整理を含む）、写真とアルバムの情報の書き出しがあります。どちらも member 全員が使えます。API・backup・manifest の形式は変わりません。
