...
同じ file では変わらない失敗（100MB 超、途中で切れた file、decode できない HEIC、server が original そのものを拒否した場合）は再試行の対象から外し、理由を出します。権限や設定のように、file ではなく画面そのものが拒否された場合も同じ扱いです。ログインを確認できなかった場合だけは、一時的な原因も含むため再試行を残し、選んだ写真を失わずに続けられる手順を出します。duplicate は失敗ではなく通常の結果のままです。
