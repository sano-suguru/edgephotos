...
OS が dark のとき（`prefers-color-scheme: dark`）は、`styles.css` が上の表の token だけを dark の値に置き換えます。stage token と `favorite` は両方で同じです。dark では既存の影だけでは dialog と info toast の境界が弱いため、縁に hairline を出します。テーマを切り替える UI は持ちません。
