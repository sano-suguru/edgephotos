...
export type BulkOutcome = 'ok' | 'gone' | 'skipped' | 'failed'
...
export async function runBulk(
    ...
    ids.map((id, index) =>
      slot(async () => {
        try {
          ...
          // The first failure speaks for the rest: they are nearly always the same problem, and the reader
          // acts on one sentence, not on a list.
          if (outcome === 'failed' && message === null) message = userMessage(err)
        }
      }),
    ),
  )
  ...
  return { ok: of('ok'), gone: of('gone'), skipped: of('skipped'), failed: of('failed'), message }
}
