...
const bodies = { original: new Blob(['o']), thumbnail: new Blob(['t']), preview: new Blob(['p']) }
...
function fake(finalizeResults: (UploadFinalizeResult | Error)[]) {
  ...
  return { deps, log, remembered: () => remembered }
}
...
describe('transferPhoto', () => {
  it('uploads everything once and forgets the reservation when done', async () => {
    const f = fake([])
    expect(await transferPhoto(f.deps, bodies, null, () => {})).toBe(ok)
    expect(f.log).toEqual(['reserve', 'put 1/original', 'put 1/thumbnail', 'put 1/preview', 'finalize upload-1'])
  ...
  it('keeps the reservation when finalize keeps failing, and a retry registers it without sending bytes', async () => {
    ...
    await expect(transferPhoto(f.deps, bodies, null, () => {})).rejects.toThrow()
    const kept = f.remembered()
    expect(kept?.upload.id).toBe('upload-1')

    f.log.length = 0
    expect(await transferPhoto(f.deps, bodies, kept, () => {})).toBe(ok)
  ...
  it('sends only the objects the server is missing while the URLs are valid', async () => {
    ...
    expect(await transferPhoto(f.deps, bodies, reservation(7), () => {})).toBe(ok)
    expect(f.log).toEqual(['finalize upload-7', 'put 7/thumbnail', 'finalize upload-7'])
  })
  ...
  it('starts over when the earlier reservation expired, was removed, or was rejected', async () => {
    for (const [previous, error] of [
      ...
      const f = fake([error])
      expect(await transferPhoto(f.deps, bodies, previous, () => {})).toBe(ok)
  ...
  it('keeps the earlier reservation when the retry cannot reach the server', async () => {
    ...
    await expect(transferPhoto(f.deps, bodies, previous, () => {})).rejects.toThrow('offline')
    expect(f.remembered()).toBe(previous)
    expect(f.log.filter((l) => l === 'reserve')).toEqual([])
  })
})
